...
class esxiServer:
    ...
    def __init__(self, configDictionary, logFile = "defaultLogfile.log"):
        """
        IN MOST USE CASES, I JUST EXPECT A JSON FILE, SO I MADE AN OPTIONAL 
        CONSTRUCTOR THAT TAKESA DICTIONARY GENERATED BY A JSON FILE
        """
        
        try:
            self.hostname =     configDictionary['HYPERVISOR_HOST']
            self.username =     configDictionary['HYPERVISOR_USERNAME']
            self.password =     configDictionary['HYPERVISOR_PASSWORD']
            self.port =         configDictionary['HYPERVISOR_LISTENING_PORT']
        except KeyError as e:
            print "CONFIG FILE DID NOT CONTAIN ALL REQUIRED DATA: " + str(e)
            return None
        self.logFile =      logFile
        ...
        self.fullName =     ""
        self.connection =   None
        if not self.logMsg("INSTANTIATING NEW HYPERVISOR OBJECT FOR HOST: " + str(self.hostname)):
            return None
        if not self.connect():
            return None
        return None
